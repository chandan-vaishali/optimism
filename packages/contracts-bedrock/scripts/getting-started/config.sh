...
fetch_block_info() {
    local response=$(curl -s -X POST -H "Content-Type: application/json" --data '{"jsonrpc":"2.0","method":"eth_getBlockByNumber","params":["latest",true],"id":1}' "$1")
    local timestamp=$(echo "$response" | jq -r '.result.timestamp')
    local hash=$(echo "$response" | jq -r '.result.hash')
    
    if [ "$timestamp" == "null" ] || [ "$hash" == "null" ]; then
        echo "Error: Failed to fetch block information"
        exit 1
    fi
...
# Fetch block info
block_info=$(fetch_block_info "$L1_RPC_URL")
block_timestamp=$(echo "$block_info" | awk '{ print $1 }')
block_hash=$(echo "$block_info" | awk '{ print $2 }')

# Print out intermediate variables for debugging
echo "block_timestamp: $block_timestamp"
echo "block_hash: $block_hash"

# Generate the config file
config=$(cat << EOL
{
  "l1StartingBlockTag": "$block_hash",
  ...
  "l2OutputOracleStartingTimestamp": "$block_timestamp",
